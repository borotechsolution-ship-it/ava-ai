...
- `password_hash`: a SHA-256 fingerprint of the password.
...
values (
  'sales-a',
  'Bret',
  encode(digest('Bret@0011', 'sha256'), 'hex'),
  'a'
);
```
...
insert into sales_accounts(login_slug, display_name, password_hash, gemini_key_slot)
values
  ('sales-a', 'Bret', encode(digest('Bret@0011', 'sha256'), 'hex'), 'a'),
  ('sales-b', 'Sales B', encode(digest('SalesB@0011', 'sha256'), 'hex'), 'b'),
  ('sales-c', 'Sales C', encode(digest('SalesC@0011', 'sha256'), 'hex'), 'c');
```
...
set
  password_hash = encode(digest('NewPassword@0011', 'sha256'), 'hex'),
  updated_at = now()
...
- `password_hash` is not the real password. It is a one-way SHA-256 fingerprint.
- `SALES_AUTH_SECRET` must stay private in `.env` and in deployment environment variables.
